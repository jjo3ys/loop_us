...
def main_load(request):
    ...
    if request.GET['last'] == '0':
        picked = Post.objects.filter(user_id=1038).select_related('user', 'project').prefetch_related('contents_image', 'contents_link', 'contents_file', 'comments__cocomments', 'post_like', 'post_tag').order_by('-id')
        ...
        post_obj = picked.union(post_obj)
